...
class EndpointTest extends \PHPUnit\Framework\TestCase
{
	...
	public function testEndpointMailer(){
		...
		if(empty($config['email']['email_rrhh'])){
			$this->fail('No esta seteado un email de destino en "mail.php" con la variable "email_rrhh"');
			throw new \Exception('No esta seteado un email de destino en "mail.php" con la variable "email_rrhh"', 1);
		}
		...
		$Email->CharSet		= 'utf8';
		$Email->isHTML();
		$Email->agregarDestinatario($config['email']['email_rrhh'], 'Test Unitario SIGARHU');
	...
	public function testEndpointPHPMailer_2(){
		...
		$email->set_asunto("Test Unitario SIGARHU testEndpointPHPMailer_2")
			->set_contenido("nada por aqui, solo un test unitario con \App\Helper\Email()", false)
	 		->set_destinatario($config['email']['email_rrhh'])
			 ->enviar();
